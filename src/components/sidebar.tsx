...
export const Sidebar = ({ active }: { active: number | undefined }) => {
    ...
    return (
     <div className="flex flex-row justify-between items-center z-20 w-full bg-green-800 h-5 fixed top-0">
      <div className="ml-8">
       <Link href={`/`}>
        <div className="inline-flex items-center cursor-pointer">
         <div className="w-8 h-8 mb-0.5 mr-1 fill-green-300">
         </div>{" "}
         <span className="font-bold mr-8 text-lg text-white">BlockSite</span>
        </div>
       </Link>
      </div>
      <div className="mr-8">
       <Button icon={homeIcon} index={0} active={active} href="/" text="Home" />
       <Button icon={usersIcon} index={3} active={active} href="/profile" text="Profile" />
       <Button icon={usersIcon} index={2} active={active} href="/users" text="Friends" />
       <WalletMultiButton className="ml-4 hover:bg-green-600 py-3 btn1 px-5 inline-flex items-center" />
      </div>
        ...
        <Link href="/profile">
         <div className="inline-flex items-center cursor-pointer">
          <div className="pb- pr-2">
           <img className="w-10 h-10 rounded-full" src={programContext.state.user.image ? programContext.state.user.image : "/img.png"} />
          </div>
          <span className="truncate text-2xl hover:text-green-400">{programContext.state.user.username}</span>
         </div>
        </Link>
        <svg
         onClick={() => {
         ...
         xmlns="http://www.w3.org/2000/svg"
         className="w-6 ml-2 h-6 hover:fill-green-400 fill-green-300"
         viewBox="0 0 512 512">
